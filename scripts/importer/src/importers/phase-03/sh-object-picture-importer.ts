/**
 * SH Object Picture Importer
 *
 * Imports pictures from mwnf3_sharing_history.sh_object_images.
 *
 * Strategy:
...
interface ShPictureGroup {
  ...
  image_number: number;
  path: string;
  translations: ShLegacyObjectImageText[];
}
...
export class ShObjectPictureImporter extends BaseImporter {
  ...
  async import(): Promise<ImportResult> {
    ...
    try {
      ...
      // Query all pictures
      const pictures = await this.context.legacyDb.query<ShLegacyObjectImage>(
        `SELECT * FROM mwnf3_sharing_history.sh_object_images 
      ...
      // Query picture texts
      let pictureTexts: ShLegacyObjectImageText[] = [];
      try {
        pictureTexts = await this.context.legacyDb.query<ShLegacyObjectImageText>(
          `SELECT * FROM mwnf3_sharing_history.sh_object_image_texts 
           ORDER BY project_id, country, number, type, image_number, lang`
        );
      } catch {
        this.logWarning('sh_object_image_texts table not found or empty');
      }

      if (pictures.length === 0) {
        this.logInfo('No SH object pictures found');
        return result;
      }
      ...
      const message = error instanceof Error ? error.message : String(error);
      result.errors.push(`Failed to query SH object pictures: ${message}`);
  ...
  private groupPictures(
    pictures: ShLegacyObjectImage[],
    pictureTexts: ShLegacyObjectImageText[]
  ): ShPictureGroup[] {
    // Create text map
    const textMap = new Map<string, ShLegacyObjectImageText[]>();
  ...
  private async importPicture(group: ShPictureGroup, _result: ImportResult): Promise<boolean> {
    ...
    if (this.isDryRun || this.isSampleOnlyMode) {
      this.logInfo(
        `[${this.isSampleOnlyMode ? 'SAMPLE' : 'DRY-RUN'}] Would import SH object picture: ${backwardCompat}`
      );
      this.registerEntity(`sample-image-sh-${backwardCompat}`, imageKey, 'image');
      return true;
    }
