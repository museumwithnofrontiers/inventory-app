...
describe('ItemExporter — deterministic list order (#1916)', () => {
  ...
  const itemRow = {
    id: 'item-a',
    type: 'monument',
    internal_name: 'Monument A',
    backward_compatibility: 'mwnf3:monuments:BAR:en:Mus01:1',
    parent_id: null,
    partner_id: null,
    country_id: null,
    collection_id: null,
    project_id: 'bar-project-uuid',
  ...
  const context = (db: Database): ExportContext => ({
    db,
    outputDir,
    projectIds: ['bar-project-uuid'],
    contextIds: ['bar-context-uuid'],
    projectKeys: ['BAR'],
  ...
  beforeEach(() => {
    outputDir = mkdtempSync(join(tmpdir(), 'baroqueart-deterministic-order-'))
    queries = []
  })
  ...
  it('orders monument-detail translations by item_id, context_id', async () => {
    const db: Database = {
      query: async (sql: string) => {
        queries.push(sql)
        if (sql.includes('ORDER BY type, display_order, internal_name')) return [itemRow]
        if (sql.includes("type = 'detail'")) {
          return [
            {
              id: 'detail-1',
              internal_name: 'Detail A',
    ...
    expect(
      queries.some(
        sql => sql.includes('FROM item_translations') && sql.includes('ORDER BY item_id, context_id')
      )
    ).toBe(true)
  })
