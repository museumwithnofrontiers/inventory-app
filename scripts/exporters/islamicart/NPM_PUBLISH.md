...
1. Exports the dataset to `output/islamicart/`
2. Bumps the patch version persisted in `output/.version-islamicart`
   (1.0.25 → 1.0.26, …)
...
The version counter lives in `output/.version-islamicart` — deliberately
*outside* the package directory, so `--force` (which deletes and recreates
`output/islamicart/`) does not reset it.
...
```bash
docker compose --profile jobs run --rm exporter islamicart --force --publish
```
