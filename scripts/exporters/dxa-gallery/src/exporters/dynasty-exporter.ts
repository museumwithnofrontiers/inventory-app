 ...
 *
 * Scoped to dynasties the gallery's own items reference. This differs from the
 * islamicart exporter, which ships the whole table because the Discover Islamic
 * Art site has a dynasty browser listing every defined dynasty; a DXA gallery
...
export class DynastyExporter extends BaseExporter {
  ...
  async export(): Promise<ExportResult> {
    ...
    if (this.memberItemIds.length === 0) {
      ...
      this.logger.warning('dynasties.json (0 — gallery has no member items)')
    ...
    const [translations, images] = await Promise.all([
      ...
      // content this gallery actually shows.
