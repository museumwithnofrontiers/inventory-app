 ...
 * (a site's languages may include Arabic, in which case the viewer must render
 * RTL), but an individual record offers whatever languages it happens to have
 * — the sheet's language switcher is built from the record, not the site — and
 * a gallery's borrowed items and partners routinely add languages the site's
 * own UI never shipped in. Both are needed, and nothing else is: exporting the
 * whole `languages` table would ship rows of names the site can never show.
 */
