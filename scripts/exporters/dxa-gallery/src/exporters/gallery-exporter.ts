 ...
 *
 * Legacy kept this in three places at once: a per-deployment `.env`
 * constraint, the `thg_gallery` row, and `thg_gallery_url`. A data package is
 * a pre-scoped API instance, so all of it collapses into one file.
 */
 ...
 * the data agrees: 'A' marks the small hand-picked set of featured galleries,
 * while status='H' marks the junk and unpublished rows (curiosities, unclear,
 * doubts, excluded). At least one gallery is status='H' + featured='A', which
 * is only possible because the two are orthogonal.
 *
 ...
 * flipping the polarity, so its JSON reports the inverse of the truth in both
 * directions: a hand-picked gallery (featured='A') answers `featured: false`,
 * and a non-featured one (featured='H') answers `featured: true`.
