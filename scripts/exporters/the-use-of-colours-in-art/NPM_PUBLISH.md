...
`@museumwnf/the-use-of-colours-in-art-data` on npmjs. What the package *contains* is
...
- **name** — `@museumwnf/the-use-of-colours-in-art-data` (hardcoded in the exporter CLI)
...
```bash
npm install @museumwnf/the-use-of-colours-in-art-data
```

```javascript
import manifest from '@museumwnf/the-use-of-colours-in-art-data/manifest.json' assert { type: 'json' }
import exhibition from '@museumwnf/the-use-of-colours-in-art-data/exhibition.json' assert { type: 'json' }
import items from '@museumwnf/the-use-of-colours-in-art-data/items.json' assert { type: 'json' }

// Lazy-load translations for a language
const { default: t } = await import(`@museumwnf/the-use-of-colours-in-art-data/translations/items.${lang}.json`)
```
