...
import type { Exhibition, ExportContext, Partner } from '../../src/core/types.js'
 ...
 * project-scoped exporters read, scoped to the exhibition's own single
 * project, plus their empty defaults so a partner the hierarchy has nothing
 * to say about still reports a value rather than an absent key.
 */
describe('PartnerExporter — shared partner shape', () => {
  ...
  const exhibition = (projectId: string | null): Exhibition => ({
    ...
    host: 'https://colours.museumwnf.org',
  ...
  beforeEach(() => {
    outputDir = mkdtempSync(join(tmpdir(), 'colours-partner-shape-'))
    queries = []
  })
  ...
  it('derives level and parent_id from the curated partner_group hierarchy', async () => {
    ...
    await new PartnerExporter(context(db, 'project-colour-uuid')).export()
  ...
  it('scopes the hierarchy query to the exhibition own single project, not a list', async () => {
  ...
  it('skips the hierarchy query entirely when the exhibition has no native project', async () => {
    const db = stubDb({ partners: [partnerRow('partner-a', 1)] })
    await new PartnerExporter(context(db, null)).export()
  ...
  it('derives project_ids from the legacy project of each held item', async () => {
    ...
    const db = stubDb({
      ...
      heldItems: [
        { partner_id: 'partner-a', item_id: 'item-a', project_id: 'project-colour-uuid' },
        { partner_id: 'partner-a', item_id: 'item-b', project_id: 'project-isl-uuid' },
      ],
    })
    await new PartnerExporter(context(db, 'project-colour-uuid', itemProjectKeys)).export()

    expect(readOutput()[0]?.project_ids?.slice().sort()).toEqual(['EXHCOLOUR', 'ISL'])
  })
  ...
  it('reports the exhibition own project for an MWNF-384 partner holding no item', async () => {
    const db = stubDb({ partners: [partnerRow('partner-orphan', 0)] })
    await new PartnerExporter(context(db, 'project-colour-uuid')).export()

    expect(readOutput()[0]?.project_ids).toEqual(['EXHCOLOUR'])
    expect(readOutput()[0]?.project_uuids).toEqual(['project-colour-uuid'])
  })
})
