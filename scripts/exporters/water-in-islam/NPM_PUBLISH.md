...
`@museumwnf/water-in-islam-data` on npmjs. What the package *contains* is
...
- **name** — `@museumwnf/water-in-islam-data` (hardcoded in the exporter CLI)
...
```bash
npm install @museumwnf/water-in-islam-data
```

```javascript
import manifest from '@museumwnf/water-in-islam-data/manifest.json' assert { type: 'json' }
import exhibition from '@museumwnf/water-in-islam-data/exhibition.json' assert { type: 'json' }
import items from '@museumwnf/water-in-islam-data/items.json' assert { type: 'json' }

// Lazy-load translations for a language
const { default: t } = await import(`@museumwnf/water-in-islam-data/translations/items.${lang}.json`)
```
