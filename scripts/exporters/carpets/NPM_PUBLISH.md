...
`@museumwnf/carpets-data` on npmjs. What the package *contains* is
...
```bash
docker compose --profile jobs run --rm exporter carpets --force --publish
```

That single run does everything:

1. Exports the gallery to `output/carpets/`
2. Bumps the patch version persisted in `output/.version-carpets`
   (1.0.0 → 1.0.1, …)
3. Generates `package.json` and a consumer `README.md` inside `output/carpets/`
...
The version counter lives in `output/.version-carpets` — deliberately
*outside* the package directory, so `--force` (which deletes and recreates
`output/carpets/`) does not reset it.
...
- **name** — `@museumwnf/carpets-data` (hardcoded in the exporter CLI)
...
```bash
npm install @museumwnf/carpets-data
```

```javascript
import manifest from '@museumwnf/carpets-data/manifest.json' assert { type: 'json' }
import gallery from '@museumwnf/carpets-data/gallery.json' assert { type: 'json' }
import items from '@museumwnf/carpets-data/items.json' assert { type: 'json' }

// Lazy-load translations for a language
const { default: t } = await import(`@museumwnf/carpets-data/translations/items.${lang}.json`)
```
