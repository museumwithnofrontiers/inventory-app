# The Use of Colours in Art (instance the-use-of-colours-in-art)

The package `@museumwnf/the-use-of-colours-in-art-data` is produced by
[`dxa-exhibition`](../dxa-exhibition/README.md) from
[`the-use-of-colours-in-art.json`](the-use-of-colours-in-art.json) next to
this file — see
[`../docs/dxa-exhibition-data-package.md`](../docs/dxa-exhibition-data-package.md)
for the package specification and
[`../dxa-exhibition/NPM_PUBLISH.md`](../dxa-exhibition/NPM_PUBLISH.md) for
the publish mechanics. This note keeps everything specific to this site:
legacy scope, membership reasoning, decisions and known gaps.

It replaces one legacy deployment of `dxa-api` + `dxa-client`:
<https://exhibitions.museumwnf.org/the_use_of_colours_in_art/en>. This was
the first of the DXA **exhibition** sites
([epic #1539](https://github.com/museumwithnofrontiers/inventory-app/issues/1539),
[story #1546](https://github.com/museumwithnofrontiers/inventory-app/issues/1546)), originally
built as a fork of [`carpets`](carpets.md)
([#1544](https://github.com/museumwithnofrontiers/inventory-app/issues/1544)); it now exports
...
Everything in a gallery package applies unchanged — membership union, facet
tags, partners, glossary, countries, languages, the worldwide timeline — so
the scoping rules are inherited from the gallery family rather than
re-derived. Three things are new, and one thing the specification said
...
## Four things this site gets right that its inputs did not
...
`us/Mus51` by name, with no explanation, from every DXA partner list. Amulets
and carpets recorded the rule and skipped it because it matched nobody there.
Here both rows hold member items — six and seventeen — so `PartnerExporter`
reproduces it (`EXCLUDED_PARTNER_KEYS`). Without it the package ships 77
museums where legacy shows 75.
...
The instance slug and package are `the-use-of-colours-in-art` /
`@museumwnf/the-use-of-colours-in-art-data` — the kebab-cased legacy slug fixed
by decision Q4. The data keeps the underscore form
...
- **`featured` disagrees with the live API, as on carpets.** dxa-api builds its
  ...
  [`../dxa-exhibition/src/exporters/exhibition-exporter.ts`](../dxa-exhibition/src/exporters/exhibition-exporter.ts).

## Validation

[`../docs/validation/the-use-of-colours-in-art-2026-08-28.md`](../docs/validation/the-use-of-colours-in-art-2026-08-28.md)
