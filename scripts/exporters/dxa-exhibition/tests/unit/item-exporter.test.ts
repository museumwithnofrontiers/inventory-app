...
/**
 * Every `related_items` stub — a link to an item outside the exhibition,
 * which can only be shipped as a reference (decision Q3) — carries the
 * target's own `project_id` (UUID). The legacy `project_key` twin it
 * originally shipped alongside (epic #1727 phase 2) is gone as of the
 * cleanup wave; these cases pin its absence next to member items and
 * related-item stubs alike.
 */
describe('ItemExporter — project_id', () => {
  ...
  beforeEach(() => {
    outputDir = mkdtempSync(join(tmpdir(), 'colours-item-exporter-'))
    queries = []
  })
  ...
  it('carries the item own project_id (UUID), with no legacy project_key', async () => {
    ...
    expect(item?.project_id).toBe('exhcolour-project-uuid')
