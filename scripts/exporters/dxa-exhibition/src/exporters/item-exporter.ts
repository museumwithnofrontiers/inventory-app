 ...
 *
 * Unlike the single-project exporters, the item universe here is the
 * exhibition's membership union, and the mix of native and borrowed records
 * in it — and the set of source projects contributing to it — is a property
 * of the curated collection, not of the exporter. Borrowed members can
 * outnumber native ones by a wide margin, or the reverse; either way the
 * source project printed on the sheet and the context that selects a
 * record's canonical translation are genuinely per-item here, not a
 * per-export constant with one exception.
 */
