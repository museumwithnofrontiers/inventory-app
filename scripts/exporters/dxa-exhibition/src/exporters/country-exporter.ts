 ...
 *
 * Omitting (3) is a real gap, not a theoretical one: a viewer that skips it
 * falls back to `Intl.DisplayNames` for every timeline-only country, and on
 * more than one site the three sets only partially overlap — the timeline
 * routinely names countries that hold or produced none of the site's own
 * items.
 *
...
export class CountryExporter extends BaseExporter {
  ...
  async export(): Promise<ExportResult> {
    ...
    if (this.memberItemIds.length === 0) {
      ...
      this.logger.warning('countries.json: exhibition has no member items — timeline countries only')
