...
export class Database {
   ...
   * by its UUID — the instance file is the only place a legacy id appears, and
   * it is resolved to this UUID ahead of time, once, via `php artisan
   * importer:find-collection`. Everything the site shows hangs off the row:
   ...
   */
  async resolveExhibition(collectionId: string): Promise<Exhibition> {
    ...
    if (!row) {
      throw new Error(
        `Exhibition collection not found: ${collectionId}\n` +
          `Run: php artisan importer:find-collection exhibition <legacy id|slug|exact English title> --json ` +
          `to look up the right collection UUID.`
      )
    }

    if (row.type !== 'exhibition') {
      throw new Error(
        `${collectionId} is a '${row.type}', not an 'exhibition'. ` +
          `Galleries have no curated theme layer — use scripts/exporters/dxa-gallery instead.`
      )
    }
   ...
   * per-language — `enabled` disagrees between languages on more than one
   * exhibition — so it is kept as a map keyed by language id.
   */
   ...
   * be conflated: on some exhibitions the top-level theme ids skip numbers or
   * run out of sequence entirely, so the rule has to be inherited rather than
   * re-derived from whichever site is in front of you.
   */
   ...
   * format — an exhibition's own project may hold a small minority of its
   * members, or most of them. So neither the project key shown on the item
   * sheet nor the context that selects its canonical translation can be a
   * per-export constant.
   */
