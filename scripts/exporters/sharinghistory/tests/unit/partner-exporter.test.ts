 ...
 * metanull/inventory-app#1699). The standalone (project-scoped) exporters
...
describe('PartnerExporter — shared partner shape', () => {
  ...
  it('derives level, parent_id and project_ids from the curated hierarchy', async () => {
    const db = stubDb({
      ...
      levels: [
        { partner_id: 'owner', level: 'partner', project_id: 'project-awe' },
        { partner_id: 'member', level: 'associated_partner', project_id: 'project-awe' },
      ],
      groupMemberships: [
  ...
  it('derives project_uuids (raw UUIDs) alongside project_ids (legacy keys)', async () => {
    const db = stubDb({
      ...
      levels: [{ partner_id: 'owner', level: 'partner', project_id: 'project-awe' }],
    })
    await new PartnerExporter(context(db)).export()

    const owner = readOutput().find(p => p.id === 'owner')
    expect(owner?.project_ids).toEqual(['awe'])
    expect(owner?.project_uuids).toEqual(['project-awe'])
  })
})
