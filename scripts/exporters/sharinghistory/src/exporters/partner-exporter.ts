...
// items.json's own scope (type IN ('object', 'monument', 'detail')) — the
// count means nothing if it disagrees with what the package actually ships
// as items.
...
export class PartnerExporter extends BaseExporter {
  ...
  async export(): Promise<ExportResult> {
    ...
    const [translations, images, logos, levels, itemCounts] = await Promise.all([
      ...
      // Same item type/project scope as items.json (ItemExporter, which here
      // also ships 'detail' as a top-level item) — item_count is meant to
      // answer "how many of the shipped items does this partner hold", so it
      // has to agree with that file, not the raw items table.
